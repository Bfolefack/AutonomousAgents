class Cell {
  ...
  void display(){
    ...
    fill(100);
    textSize(10);
    text(singleDistBetween(0, 0), gridPos.x * gridScale + gridScale/2, gridPos.y * gridScale + gridScale/2);
      
  }
  
  PVector distBetween (int x, int y){
    
    float e = dist(0, gridPos.y * gridScale, 0, y * gridScale);
    float f = dist(gridPos.x * gridScale, 0, x * gridScale, 0);
    
    return new PVector(f, e);
  }
  
  float singleDistBetween (int x, int y){
    
    float e = dist(gridPos.x * gridScale, gridPos.y * gridScale, x * gridScale, y * gridScale);
