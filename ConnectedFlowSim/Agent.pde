class Agent {
  ...
  void think(ArrayList<PVector> dodge, ArrayList<PVector> allies) {
    ...
    PVector tile = flowField[int(grdSpace.x)][int(grdSpace.y)];
  ...
  PVector getGridspace() {
    ...
    if(x < 0){
     x = 0; 
    } else if (x > flowField.length - 1){
      x = flowField.length - 1;
    }
    if(y < 0){
     y = 0; 
    } else if (y > flowField[0].length - 1){
      y = flowField[0].length - 1;
    }
